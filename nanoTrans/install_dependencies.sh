...
clean () {
    ...
    if [ -d $dir ] 
    then
	echo "skipping remove previously failed installation in $BUILD/$dir (permission workaround)"
	# rm -rf $dir
    fi
}
...
install_r_pkg () {
    if [ -z "$(check_installed "$rlib_dir/$1")" ]; then
        ...
        R -e "options(download.file.method = \"wget\"); install.packages(\"$1\", repos=\"$repo\", lib=\"$build_dir/R_libs/\")"
...
install_r_pkg_source () {
    if [ -z "$(check_installed "$rlib_dir/$1")" ]; then
        ...
        if [[ "$mainland_china_installation" == "yes" ]]; then
            base_url="https://mirrors.tuna.tsinghua.edu.cn/CRAN"
        fi
        url_source_rpkg="$base_url/src/contrib/Archive/$1/${1}_${2}.tar.gz"
        
        # Download using wget which works better with SSL here
        filename="${1}_${2}.tar.gz"
        wget -c --no-check-certificate "$url_source_rpkg" -O "$filename"
        
        # Install from local file
        R -e "install.packages(\"$filename\", repos=NULL, type=\"source\", lib=\"$build_dir/R_libs/\")"
...
install_pkg_byconda () {
    if [ ! -d $site_packages/$1 ]; then mkdir -p -p $site_packages/$1 ; fi
    if [ -z $(check_installed $site_packages/$1) ]; then
        echo "[$(timestamp)] Installing $1 ..."
        $miniconda3_dir/micromamba install -y --channel-priority flexible -p $build_dir/miniconda3 $1=$2
    fi
...
install_and_create_pkg_condaenv () {
    if [ -z $(check_installed $build_dir/${1}_conda_env/bin) ]; then
        ...
        $miniconda3_dir/conda install -y $1=$2
...
if [ -z $(check_installed $miniconda3_dir) ]; then
    ...
    bash Miniconda3-${MINICONDA3_VERSION}-Linux-x86_64.sh -b -p build/miniconda3
...
#     $miniconda3_dir/conda install -y nanofilt=${NANOFILT_VERSION}
#     source $miniconda3_dir/deactivate
...
if [ -z $(check_installed $nanoplot_dir) ]; then
    cd $build_dir
    $miniconda3_dir/micromamba create -y -p $build_dir/nanoplot_conda_env python=3.9 -c conda-forge -c bioconda
    # source $miniconda3_dir/activate $build_dir/nanoplot_conda_env
    $miniconda3_dir/micromamba install -y -p $build_dir/nanoplot_conda_env nanoplot=${NANOPLOT_VERSION} -c conda-forge -c bioconda
    # source $miniconda3_dir/deactivate
...
if [ -z $(check_installed $picard_dir) ]; then
    ...
    mkdir -p Picard-v${PICARD_VERSION}
    cp -r picard.jar $picard_dir
    cd $picard_dir
...
if [ -z $(check_installed $gffread_dir) ]; then
    ...
    cp -r gffread-${GFFREAD_VERSION}.Linux_x86_64 gffread
    cd $build_dir
...
if [ -z $(check_installed $nanopolish_dir) ]; then
    ...
    $miniconda3_dir/micromamba create -y -p $build_dir/nanopolish_conda_env python=3.9 -c conda-forge -c bioconda
    # source $miniconda3_dir/activate $build_dir/nanopolish_conda_env
    $miniconda3_dir/micromamba install -y -p $build_dir/nanopolish_conda_env nanopolish=${NANOPOLISH_VERSION} -c conda-forge -c bioconda
    # source $miniconda3_dir/deactivate
...
if [ -z $(check_installed $xpore_dir) ]; then
    ...
    $miniconda3_dir/micromamba create -y -p $build_dir/xpore_conda_env python=3.9 -c conda-forge -c bioconda
    # source $miniconda3_dir/activate $build_dir/xpore_conda_env
    # $miniconda3_dir/conda install -y xpore=${XPORE_VERSION}
    $miniconda3_dir/micromamba run -p $build_dir/xpore_conda_env pip install xpore==${XPORE_VERSION}
    # source $miniconda3_dir/deactivate
...
if [ -z $(check_installed $flair_dir) ]; then
    ...
    $miniconda3_dir/micromamba create -y -p $build_dir/flair_conda_env python=3.9 -c conda-forge -c bioconda
    # source $miniconda3_dir/activate $build_dir/flair_conda_env
    $miniconda3_dir/micromamba install -y -p $build_dir/flair_conda_env flair=${FLAIR_VERSION} -c conda-forge -c bioconda
    # source $miniconda3_dir/deactivate
    # fix a bug of flair
    cd $flair_dir
...
# rm bam2Bed12.content
    ...
    # cp -f $NANOTRANS_HOME/misc/plot_isoform_usage_for_pdf plot_isoform_usage
    cat $NANOTRANS_HOME/misc/plot_isoform_usage_for_pdf > plot_isoform_usage
    chmod +x plot_isoform_usage
...
# tabix is already installed as part of samtools in flair conda environment
...
#fi
install_pkg_byconda fastx_toolkit $FASTXTK_VERSION
fastxtk_dir=$miniconda3_dir
...
# rm JAFFA-version-${JAFFAL_VERSION}.tar.gz
    ...
    cat $jaffal_dir/known_fusions.txt > $NANOTRANS_HOME/misc/human.known_fusions.txt
