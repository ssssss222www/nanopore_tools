...
test_read_file <- function(filepath) {
  tryCatch({
    fread(filepath) %>% 
      mutate(across(where(is.numeric), sprintf, fmt = '%.4f'))
  }, error = function(e) {
...
test_print_plots <- function(filepath) {
  if(length(filepath) > 0) {
    # Check if file is PDF, if so convert to PNG or check if PNG exists
    valid_paths <- c()
    for (f in filepath) {
        if (grepl("\\.pdf$", f, ignore.case = TRUE)) {
            # Copy to temp dir to avoid permission issues and clutter
            local_pdf <- basename(f)
            file.copy(f, local_pdf, overwrite = TRUE)
            
            # Use local png name (remove .pdf extension properly)
            local_png_prefix <- sub("\\.pdf$", "", local_pdf, ignore.case = TRUE)
            local_png <- paste0(local_png_prefix, ".png")
            
            # Convert using pdftoppm
            # pdftoppm -png -singlefile -r 150 input.pdf output_prefix
            cmd <- paste("pdftoppm -png -singlefile -r 150", shQuote(local_pdf), shQuote(local_png_prefix))
            system(cmd)
            
            if (file.exists(local_png)) {
                valid_paths <- c(valid_paths, local_png)
            } else {
                # Fallback to original if conversion fails
                valid_paths <- c(valid_paths, f)
            }
        } else {
            valid_paths <- c(valid_paths, f)
        }
...
dt_outputs <- data.table(
  ...
  Module_03_dsi_table     = c("03.Isoform_Expression_and_Splicing_Comparison", batch_id, "table",  ".*.differential_isoform_splicing.all.events.quant.tidy.txt$"),
  Module_03_deg_volcano   = c("03.Isoform_Expression_and_Splicing_Comparison", batch_id, "figure", ".*.differential_gene_volcanoplot.pdf$"),
  Module_03_dei_volcano   = c("03.Isoform_Expression_and_Splicing_Comparison", batch_id, "figure", ".*.differential_isoform_volcanoplot.pdf$"),
  ...
  Module_04_mod_char_figs = c("04.Isoform_RNA_Modification_Identification",    batch_id, "figure", ".*.rna_modification.diffmoddiff_RNA_mod.*.top20.pdf$"),
  ...
  Module_05_polya_figs    = c("05.Isoform_PolyA_Tail_Length_Profiling",        batch_id, "figure", ".*.polya_profiling.*.density_plot.pdf$|.*.polya_profiling.*.violin_plot.pdf$"),
...
if(!is.character(dt_degs)) datatable(dt_degs) 
 #%>% formatRound(5:7, 2)
```
## Differentially Expressed Isoforms (DEIs)
```{r isoform-comparison-table}
dt_deis <- test_read_file(outputs_list$Module_03_dei_table)
if(!is.character(dt_deis)) datatable(dt_deis) 
#%>% formatRound(5:7, 2)
```
## Differential Usage of Isoforms
```{r isoform-comparison-usage-table}
dt_dius <- test_read_file(outputs_list$Module_03_diu_table)
if(!is.character(dt_dius)) datatable(dt_dius) 
#%>% formatRound(5:8, 2)
```
## Differential Splicing Events
```{r isoform-comparison-splicing-table}
dt_dsis <- test_read_file(outputs_list$Module_03_dsi_table)
if(!is.character(dt_dsis)) datatable(dt_dsis) 
#%>% formatRound(5:8, 2)
```
:::
...
if(length(outputs_list$Module_06_genefus_figs) > 0) {
  print("Here is an example of a fusion gene. If you would like to know about all the detected fusion genes, please look in the module 06 folder.")
