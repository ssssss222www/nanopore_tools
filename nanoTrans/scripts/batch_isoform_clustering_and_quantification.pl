...
my $ref_genome_file = "$ref_dir/ref.genome.fa";
if ($ref_dir !~ /^\//) {
    $ref_genome_file = "$base_dir/$ref_dir/ref.genome.fa";
}
my $ref_genome_annotation_file = "$ref_dir/ref.genome.gtf";
if ($ref_dir !~ /^\//) {
    $ref_genome_annotation_file = "$base_dir/$ref_dir/ref.genome.gtf";
}
my $ref_transcriptome_file = "$ref_dir/ref.transcriptome.fa";
if ($ref_dir !~ /^\//) {
    $ref_transcriptome_file = "$base_dir/$ref_dir/ref.transcriptome.fa";
}

foreach my $sample_id (@sample_table) {
    my $basecalled_fastq_file = "$sample_table{$sample_id}{'basecalled_fastq_file'}";
    if ($basecalled_fastq_file !~ /^\//) {
        $basecalled_fastq_file = "$base_dir/$long_reads_dir/$basecalled_fastq_file";
    }
    my $basecalled_fast5_dir = "$sample_table{$sample_id}{'basecalled_fast5_dir'}";
    if ($basecalled_fast5_dir !~ /^\//) {
        $basecalled_fast5_dir = "$base_dir/$long_reads_dir/$basecalled_fast5_dir";
    }
    $sample_count++;
    
    my $bam_file = "$mapping_dir/$batch_id/$sample_id/$sample_id.sort.bam";
    if ($mapping_dir !~ /^\//) {
         $bam_file = "$base_dir/$mapping_dir/$batch_id/$sample_id/$sample_id.sort.bam";
    }
	...
	system("$flair_dir/flair correct -t $threads -q $sample_id.bam2bed.bed12 -g $ref_genome_file --nvrna -f $ref_genome_annotation_file -o $sample_id.flair");
...
foreach my $sample_id (@sample_table) {
    ...
    my $fastq = $sample_table{$sample_id}{'basecalled_fastq_file'};
    if ($fastq !~ /^\//) {
        $fastq = "$base_dir/$long_reads_dir/$fastq";
    }
    print $manifest_fh "$sample_id\t$sample_table{$sample_id}{'comparison_group'}\t$sample_table{$sample_id}{'replicate_id'}\t$fastq\n";
}

if ($debug eq "no") {
    system("$flair_dir/flair collapse -t $threads -q $batch_id.all_samples_combined.flair_all_corrected.bed -g $ref_genome_file -f $ref_genome_annotation_file -r $basecalled_fastq_file_array_list --isoformtss --no_redundant none --generate_map --annotation_reliant generate --quality $min_mapping_quality -o $batch_id.all_samples_combined.flair_all_collapsed --temp_dir ./tmp");
} else {
    system("$flair_dir/flair collapse -t $threads -q $batch_id.all_samples_combined.flair_all_corrected.bed -g $ref_genome_file -f $ref_genome_annotation_file -r $basecalled_fastq_file_array_list --isoformtss --no_redundant none --generate_map --annotation_reliant generate --quality $min_mapping_quality -o $batch_id.all_samples_combined.flair_all_collapsed --keep_intermediate --temp_dir ./tmp");
}
...
my $sample_table_path = $sample_table;
if ($sample_table !~ /^\//) {
...
sub parse_sample_table {
	...
	/^\s*$/ and next;
        ...
        next if $sample_id eq 'sample_id'; # Skip header line
