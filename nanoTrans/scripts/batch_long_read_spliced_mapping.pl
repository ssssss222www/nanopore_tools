...
# DEBUG: Check parsed sample table
# foreach my $id (@sample_table) {
#     my $fastq = $sample_table{$id}{'basecalled_fastq_file'};
#     print "DEBUG: Sample: $id, FASTQ: $fastq\n";
#     if (!defined $fastq || $fastq eq "") {
#         die "Error: basecalled_fastq_file is empty for sample $id. Please check the sample table format.\n";
#     }
# }
...
foreach my $sample_id (@sample_table) {
    ...
    if ($basecalled_fastq_file !~ /^\//) {
        ...
        # It is absolute path, but we need to check if it exists directly or needs prefix
        if (!-e $basecalled_fastq_file) {
             # fallback: try with prefix (though unlikely for absolute path unless it's wrong)
             # But here we assume absolute path is correct.
        }
    }
	...
	if ($mapping_mode eq "map2genome") {
	    my $ref_file = $ref_genome_file;
	    if ($ref_file !~ /^\//) { $ref_file = "$base_dir/$ref_file"; }
	    system("perl $NANOTRANS_HOME/scripts/summarize_mapping_coverage.pl -r $ref_file -s $sample_id.samstat -m lite -t $sample_id -o $sample_id.coverage_summary.txt");
	} elsif ($mapping_mode eq "map2transcriptome") {
	    my $ref_file = $ref_transcriptome_file;
	    if ($ref_file !~ /^\//) { $ref_file = "$base_dir/$ref_file"; }
            system("perl $NANOTRANS_HOME/scripts/summarize_mapping_coverage.pl -r $ref_file -s $sample_id.samstat -m lite -t $sample_id -o $sample_id.coverage_summary.txt");
	}
	if ($mapping_count == 1) {
