...
foreach my $sample_id (@sample_table) {
    ...
    my $isoform_clustering_fasta_file = "$isoform_cq_dir/$batch_id/all_samples_combined/$batch_id.all_samples_combined.flair_all_collapsed.isoforms.fa";
    if ($isoform_cq_dir !~ /^\//) {
        $isoform_clustering_fasta_file = "$base_dir/$isoform_cq_dir/$batch_id/all_samples_combined/$batch_id.all_samples_combined.flair_all_collapsed.isoforms.fa";
    }
    my $isoform_clustering_gtf_file = "$isoform_cq_dir/$batch_id/all_samples_combined/$batch_id.all_samples_combined.flair_all_collapsed.isoforms.gtf";
    if ($isoform_cq_dir !~ /^\//) {
        $isoform_clustering_gtf_file = "$base_dir/$isoform_cq_dir/$batch_id/all_samples_combined/$batch_id.all_samples_combined.flair_all_collapsed.isoforms.gtf";
    }
    ...
    my $transcript2gene_map_path = $transcript2gene_map;
    if ($transcript2gene_map !~ /^\//) {
        $transcript2gene_map_path = "$base_dir/$transcript2gene_map";
    }
	system("perl $NANOTRANS_HOME/scripts/summarize_nanopolish_polya_profiling_results.pl -i $sample_id.nanopolish.polya_profiling.filtered.txt -m $transcript2gene_map_path -o $sample_id.nanopolish.polya_profiling.filtered.summary.txt");
