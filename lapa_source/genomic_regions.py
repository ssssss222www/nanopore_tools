...
class _GenomicRegions:
    ...
    def annotate(self, gr, features=None):
        ...
        # Ensure both have Strand column and it is compatible
        # Sometimes PyRanges loses Strand category info or has different categories
        common_strands = set(gr.Strand.unique()) & set(df_gtf['Strand'].unique())
        df_gtf = df_gtf[df_gtf['Strand'].isin(common_strands)]
        
        gr_gtf = pr.PyRanges(df_gtf, int64=True)
        gr_query = pr.PyRanges(gr.df, int64=True)

        # Force unstranded join to avoid "Can only do stranded operations when both PyRanges contain strand info"
        # We will filter by strand manually if needed or rely on unstranded join logic
        try:
             gr_ann = gr_query.join(gr_gtf, strandedness='same', how='left')
        except Exception:
             gr_ann = gr_query.join(gr_gtf, strandedness=False, how='left')
             # If we do unstranded join, we should filter by strand manually to match 'same' behavior
             if 'Strand_b' in gr_ann.columns:
                 gr_ann = gr_ann[gr_ann.Strand == gr_ann.Strand_b]

        if 'Start_b' in gr_ann.columns:
            gr_ann = gr_ann.drop(['Start_b', 'End_b', 'Strand_b'])
