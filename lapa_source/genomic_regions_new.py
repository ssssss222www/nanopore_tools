...
class _GenomicRegions:
    ...
    def annotate(self, gr, features=None):
        ...
        # Fix for pyranges 0.1.x compatibility:
        # pyranges 0.1.x automatically adds '.' to Strand categories when
        # constructing PyRanges from a DataFrame, resulting in category mismatch
        # (['.', '-', '+'] vs ['-', '+']) between query and GTF PyRanges objects.
        # This causes "Can only do stranded operations when both PyRanges contain
        # strand info" AssertionError during strandedness='same' join.
        #
        # Fix: enforce identical Strand categories ['+', '-'] on both sides
        # before constructing PyRanges objects. Rows with '.' (unstranded reads)
        # are dropped via dropna — they cannot participate in stranded joins
        # and should not be annotated against strand-specific GTF features.
        _strand_cats = ['+', '-']

        df_query = gr.df.copy()
        df_query['Strand'] = pd.Categorical(df_query['Strand'], categories=_strand_cats)
        df_query = df_query.dropna(subset=['Strand'])

        df_gtf['Strand'] = pd.Categorical(df_gtf['Strand'], categories=_strand_cats)
        df_gtf = df_gtf.dropna(subset=['Strand'])

        gr_gtf = pr.PyRanges(df_gtf, int64=True)

        gr_ann = pr.PyRanges(df_query, int64=True).join(
